...
class Face {
  ...
  getIndexAtInternal(x: number, y: number) {
    if (x < 0) {
      x = this.size + x;
    }
    if (y < 0) {
  ...
  getAtInternal(x: number, y: number) {
    return this.data[this.getIndexAtInternal(x, y)];
  }
  setAtInternal(x: number, y: number, v: Color) {
    const i = this.getIndexAtInternal(x, y);
    this.data[i] = v;
    const pixelSize = this.canvas.width / this.size;
    this.ctx.fillStyle = i2color[v];
    this.ctx.fillRect(x * pixelSize, y * pixelSize, pixelSize, pixelSize);
...
class Cube {
  ...
  constructor(
    ...
    this.frontInternal = new Face(
      canvases[0],
      splits.slice(0, 3),
      size,
      Color.R,
    );
    this.rightInternal = new Face(
      canvases[1],
      splits.slice(3, 6),
      size,
      Color.G,
    );
    this.leftInternal = new Face(
      canvases[2],
      splits.slice(6, 9),
      size,
      Color.B,
    );
    this.backInternal = new Face(
      canvases[3],
      splits.slice(9, 12),
      size,
      Color.O,
    );
    this.topInternal = new Face(
      canvases[4],
      splits.slice(12, 15),
      size,
      Color.Y,
    );
    this.bottomInternal = new Face(
      canvases[5],
      splits.slice(15, 18),
      size,
      Color.W,
    ...
    this.front = this.frontInternal;
    this.right = this.rightInternal;
    this.left = this.leftInternal;
  ...
  updateFace() {
    ...
    const inv = invMap[this.frontDir][this.topDir]!
    const map = [inv[0] ? axis[0] + 3 : axis[0],
    inv[1] ? axis[1] + 3 : axis[1],
    inv[2] ? axis[2] + 3 : axis[2],
    inv[0] ? axis[0] : axis[0] + 3,
    inv[1] ? axis[1] : axis[1] + 3,
    inv[2] ? axis[2] : axis[2] + 3,]
    this.front = this.internals[map[0]]
    this.top = this.internals[map[1]]
    this.right = this.internals[map[2]]
    this.back = this.internals[map[3]]
    this.bottom = this.internals[map[4]]
    this.left = this.internals[map[5]]
    const imap = []
    imap[map[0]] = 0
    imap[map[1]] = 1
    imap[map[2]] = 2
    ...
    imap[map[4]] = 4
    imap[map[5]] = 5
    this.frontInternal.rotInternal = rotMap[imap[0]][imap[1]] as 0 | 1 | 2 | 3
    this.topInternal.rotInternal = rotMap[imap[1]][imap[3]] as 0 | 1 | 2 | 3
    this.rightInternal.rotInternal = rotMap[imap[2]][imap[1]] as 0 | 1 | 2 | 3
    this.backInternal.rotInternal = rotMap[imap[3]][imap[1]] as 0 | 1 | 2 | 3
    this.bottomInternal.rotInternal = rotMap[imap[4]][imap[0]] as 0 | 1 | 2 | 3
    this.leftInternal.rotInternal = rotMap[imap[5]][imap[1]] as 0 | 1 | 2 | 3
  }
    ...
    for (let j = start; j < end + 1; j++) {
      const elements: Color[][] = [
        ...
        () => {
          const ret: Color[][] = [[], [], [], []];
          for (let k = 0; k < this.size; k++) {
            ret[0][k] = this.frontInternal.getAtInternal(k, this.size - j - 1);
            ret[1][k] = this.leftInternal.getAtInternal(k, this.size - j - 1);
            ret[2][k] = this.backInternal.getAtInternal(k, this.size - j - 1);
            ret[3][k] = this.rightInternal.getAtInternal(k, this.size - j - 1);
          }
          return ret;
        },
        () => {
      ...
      [
        ...
        () => {
          for (let k = 0; k < this.size; k++) {
            this.frontInternal.setAtInternal(j, k, elements[0][k]);
            this.topInternal.setAtInternal(j, k, elements[1][k]);
            this.backInternal.setAtInternal(
              this.size - j - 1,
              this.size - k - 1,
              elements[2][k],
            );
            this.bottomInternal.setAtInternal(j, k, elements[3][k]);
          }
        },
      ][axis]();
    }
